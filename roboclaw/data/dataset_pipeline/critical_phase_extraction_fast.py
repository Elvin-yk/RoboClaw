...
def _batch_decode_interval(source_ds, ep_meta, video_keys, start_frame, end_frame):
    ...
    for vid_key in video_keys:
        ...
        backend = getattr(
            source_ds,
            "video_backend",
            getattr(source_ds, "_video_backend", None),
        )
        decoded[vid_key] = decode_video_frames(
            vpath, timestamps, source_ds.tolerance_s, backend,
        )
    return decoded
...
def extract_critical_phase_dataset_direct(
    ...
    has_ld_encode = hasattr(ld_module, "encode_video_frames")
    original_encode_ld = ld_module.encode_video_frames if has_ld_encode else None
    ...
    # Patch import sites (ld_module re-export only present in some lerobot versions)
