...
class _FakeLifecycle:
    def __init__(self, *, bringup_ok: bool = True) -> None:
        self._running = False
        self._bringup_ok = bringup_ok
    ...
    def bringup(self, **kwargs) -> dict:
        ...
        self._running = self._bringup_ok
        return {
            "ok": self._bringup_ok,
            "message": "started" if self._bringup_ok else "failed",
            "process": self.status(),
        }
...
def test_simulation_service_does_not_persist_map_paths_when_lifecycle_bringup_fails(
    tmp_path,
) -> None:
    lifecycle = _FakeLifecycle(bringup_ok=False)
    state_path = tmp_path / "simulation_state.json"
    service = SimulationService(
        lifecycle=lifecycle,
        doctor_runner=lambda profile_id: _manifest(
    ...
    assert lifecycle.bringup_calls[0]["map_path"].endswith("maps/map_house.yaml")
    loaded = load_simulation_state(state_path)
    assert loaded["paths"]["launch"] == ""
    assert loaded["paths"]["map_id"] == ""
    assert loaded["paths"]["map"] == ""
    assert loaded["paths"]["world"] == ""
    assert loaded["paths"]["semantic_graph"] == ""
